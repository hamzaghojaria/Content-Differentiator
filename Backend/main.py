...
@app.post(
    "/content-difference/"
)  # Validates multiple selected columns and returns filtered data.
async def differentiator(
    column1: str = Form(...), column2: str = Form(...), file: UploadFile = File(...)
):
    try:
        ...
        if df.shape[1] < 2:
            ...
            missing_columns = [
                col for col in selected_columns_list if col not in df.columns
            ]  # Validate Valid Columns
            if missing_columns:
                return {
                    "Error": f"Invalid columns: {missing_columns}. Available columns: {df.columns.tolist()}"
                }
            ...
            if filtered_df.shape[0] >= 1:
                ...
                filtered_df[column1] = filtered_df[column1].astype(str)
                filtered_df[column2] = filtered_df[column2].astype(str)

                filtered_df["Similarity Ratio"] = filtered_df.apply(
                    lambda row: differ_proc.similarity_ratio(
                        row[column1], row[column2]
                    ),
                    axis=1,
                )
                filtered_df["Similarity Ratio"] = filtered_df["Similarity Ratio"].round(
                    2
                )
                filtered_df["Content_Difference"] = filtered_df.apply(
                    lambda row: differ_proc.color_diff(row[column1], row[column2]),
                    axis=1,
                )
                ...
                differ_proc.color_code_sentences(
                    filtered_df, "Content_Difference", file_path
                )
